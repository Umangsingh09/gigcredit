...
from .distribution_check import compute_distribution, save_distribution
...
def save_model(
	pipeline: Pipeline, feature_names: list, out_dir: str, raw_df: pd.DataFrame
) -> str:
	...
	metadata = {
		...
		"excludes_worker_id": "worker_id" not in feature_names,
		"worker_id_exclusion_reason": (
			"worker_id is a synthetic sequential row identifier "
			"(data_generator.generate_dataset: worker_id = np.arange(1, n+1)) "
			"with no causal relationship to loan_repaid. An earlier model "
			"version (feature_names had 16 entries, worker_id first) "
			"accidentally fit on it as a scaled input, which let the "
			"arbitrary identifier value materially shift predictions and "
			"risk categories with no real information behind it. See "
			"ml-engine/models/MODEL_CARD.md."
		),
		"training_data_source": (
			"ml-engine/data/gig_workers.csv, synthetic, generated by "
			"ml-engine/src/data_generator.py"
		),
		"training_row_count": int(len(raw_df)),
		"income_scale_validated_against_production": False,
		"income_unit": (
			"undocumented/synthetic — NOT confirmed to be INR, USD, or any "
			"real currency. Do not assume a currency or apply a conversion "
			"factor without an explicit, sourced decision. See "
			"ml-engine/models/MODEL_CARD.md."
		),
	}
	...
	distribution_bounds = compute_distribution(raw_df)
	save_distribution(distribution_bounds, out_dir)
...
def main():
	...
	model_path = save_model(pipeline, list(X.columns), str(models_dir), df)
